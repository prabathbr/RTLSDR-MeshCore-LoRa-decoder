...
static char call(char modbigssid, int32_t * p,
                char raw[], uint32_t raw_len, uint32_t * i,
                char mon[], uint32_t mon_len, char sep1,
                char sep2, char sep3, uint32_t sbase)
{
   uint32_t s;
   uint32_t l;
   l = 0UL;
   ...
   if (mon[*i]=='-') {
      ...
      if (modbigssid) s = s&15UL;
      else if (s>15UL) return 0;
   }
                ...
                char modbigssid)
{
   ...
   *p = 7L;
   i = 0UL;
   if (!call(modbigssid, p, raw, raw_len, &i, mon, mon_len, '>', 0, 0,
                48UL)) {
      *p = 0L; /* from call */
      return;
   }
   *p = 0L;
   ...
   /* ">" */
   ++i;
   if (!call(modbigssid, p, raw, raw_len, &i, mon, mon_len, ':', ',', 0,
                112UL)) {
   ...
   *p = 14L;
   ...
   while (mon[i]==',') {
      ++i;
      if (!call(modbigssid, p, raw, raw_len, &i, mon, mon_len, ':', ',', '*',
                 48UL)) {
         *p = 0L;
         return;
      }
