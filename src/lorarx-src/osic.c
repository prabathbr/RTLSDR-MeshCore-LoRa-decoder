...
{
#ifdef _WIN32
	if (port_win_is_stdio_path(fn, fn_len, 1))
		return port_win_open_stdio(1, 0);
#endif
	if (FNLENCHECK && strnlen(fn, fn_len) > fn_len)
		return -1;
#ifdef _WIN32
	return open(fn, O_RDWR | O_CREAT | O_TRUNC | O_BINARY, _S_IREAD | _S_IWRITE);
#else
...
{
#ifdef _WIN32
	if (port_win_is_stdio_path(fn, fn_len, 0))
		return port_win_open_stdio(0, 0);
#endif
	if (FNLENCHECK && strnlen(fn, fn_len) > fn_len)
		return -1;
#ifdef _WIN32
...
#ifdef _WIN32
	if (port_win_is_stdio_path(fn, fn_len, 1))
		return port_win_open_stdio(1, 1);
#endif
	if (FNLENCHECK && strnlen(fn, fn_len) > fn_len)
		return -1;
#ifdef _WIN32
	return open(fn, O_RDWR | O_BINARY);
#else
...
{
#ifdef _WIN32
	__int64 rc = _lseeki64(fd, lo, whence);
#else
	off64_t rc;
...
{
#ifdef _WIN32
	if (_lseeki64(fd, rel, SEEK_CUR) < 0)
...
int osic_setttybaudraw(int32_t fd, uint32_t baud)
{
  ...
  if (baud==300) bd=B300;
  else if (baud==600) bd=B600;
  else if (baud==1200) bd=B1200;
  else if (baud==2400) bd=B2400;
  else if (baud==4800) bd=B4800;
  else if (baud==9600) bd=B9600;
  else if (baud==19200) bd=B19200;
  else if (baud==38400) bd=B38400;
  else if (baud==57600) bd=B57600;
  else if (baud==115200) bd=B115200;
  else if (baud==230400) bd=B230400;
  else if (baud==460800) bd=B460800;
  else return -1;

  res=tcgetattr(fd, &options);
  if (res<0) return res;
 
  res=cfsetispeed(&options, bd);
  if (res<0) return res;

  res=cfsetospeed(&options, bd);
  if (res<0) return res;
...
int osic_keepalive(int32_t fd, char on, int32_t idle, int32_t intervall, int32_t count)
{
  ...
  (void)count;
  ion=(int)on;
  res=setsockopt((SOCKET)fd, SOL_SOCKET, SO_KEEPALIVE, (const char *)&ion, sizeof(int));
#ifdef _WIN32
  return res;
#else
  if (res<0) return res;
  ...
  if (idle>=0) {
    res=setsockopt(fd, SOL_TCP, TCP_KEEPIDLE, &idle, sizeof(int));
    if (res<0) return res;  
  };

  if (intervall>=0) {
    res=setsockopt(fd, SOL_TCP, TCP_KEEPINTVL, &intervall, sizeof(int));
    if (res<0) return res;
  };

  if (count>=0) {
    res=setsockopt(fd, SOL_TCP, TCP_KEEPCNT, &count, sizeof(int));
    if (res<0) return res;
  };
...
{
  return (int32_t)ctime((time_t*)time0);
}
...
void osic_timens(char monotonic, uint32_t *s, uint32_t *ns)
{
  ...
  *s = (uint32_t)((uli.QuadPart / 10000000ULL) - 11644473600ULL);
  *ns = (uint32_t)((uli.QuadPart % 10000000ULL) * 100ULL);
#else
  struct timespec t;

  if (monotonic) clock_gettime(CLOCK_MONOTONIC, &t);
